<template>
  <v-container>
    <v-row>
      ...
      <v-col v-if="screenWidth < 600" cols="4" class="d-flex justify-end align-end">
        <v-menu open-on-hover>
          <template v-slot:activator="{ props }">
            <v-btn v-bind="props" class="btnMenu">
              <img src="@/assets/SVG/Bouton_menu.svg" alt="menu" class="logoMenu" />
            </v-btn>
          </template>
      ...
      <v-col v-else cols="4" class="d-flex align-end">
        <v-row class="d-flex justify-center align-end text-center">
          <v-col cols="4" class="pa-0" v-for="(item, index) in items" :key="index" :value="index">
            <router-link class="menuLink" :to="item.link">{{ item.title }}</router-link>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
